...
def login(request):
	if request.method == 'POST':
		...
		if user is not None:
			...
			messages.success(request, 'Nous sommes heureux de vous revoir !')
...
def logout(request):
	auth.logout(request)
	messages.success(request, 'Merci pour votre temps sur notre site')
	return redirect('login')
...
def forgotpassword(request):
	if request.method == 'POST':
		...
		if Account.objects.filter(email=email).exists():
			...
			messages.error(request, "Cette adresse n'est associée à aucun compte")
...
def resetpassword_validate(request, uidb64, token):
	...
	if user is not None and default_token_generator.check_token(user, token):
		...
		messages.success(request, 'Réinitialiser votre mot de passe')
...
def resetpassword(request):
	...
	if request.method == 'POST':
		...
		if password == password2:
			...
			user.save()
			messages.success(request, 'Votre mot de passe a bien été réinitialiser !')
			return redirect('login')
