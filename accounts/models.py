...
class MyAccountManager(BaseUserManager):
	def create_user(self, first_name, last_name, username, email, city, password = None):
		...
		if not username:
			raise ValueError("Vous devez fournir un pseudo")
...
class Account(AbstractBaseUser):
	first_name = models.CharField('Prenom', max_length=60, unique=True)
	last_name = models.CharField('Nom', max_length=60, unique=True)
